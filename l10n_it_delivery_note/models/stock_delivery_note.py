...
DELIVERY_NOTE_STATES = [('draft', "Draft"), ('confirm', "Validated"),
...
INVOICE_STATUSES = [('no', "Nothing to invoice"), ('to invoice', "To invoice"),
...
class StockDeliveryNote(models.Model):
    ...
    display_name = fields.Char(
        compute='_compute_display_name', store=True, index=True, copy=False
    )
            ...
            if self._update_generic_shipping_information(self.type_id):
                return {
                    'warning': {
                        'title': _("Warning!"),
                        'message':
                            "Some of the shipping configuration have "
    ...
    def _onchange_partner(self):
        ...
        if self.partner_id:
            pickings_picker_domain = [('delivery_note_id', '=', False),
    ...
    def update_detail_lines(self):
        for note in self:
            ...
            move_ids_to_create = [
                l for l in pickings_move_ids if l not in lines_move_ids
            ]
            move_ids_to_delete = [
                l for l in lines_move_ids if l not in pickings_move_ids
            ]
