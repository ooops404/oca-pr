...
class RibaUnsolved(models.TransientModel):
    ...
    def _get_effects_amount(self):
        if not self.env.context.get('active_id', False):
            return False
        return self.env['riba.distinta.line'].browse(
    ...
    def create_move(self):
        ...
        wizard = self
        if (
            not wizard.unsolved_journal_id or not wizard.effects_account_id
            or not wizard.riba_bank_account_id or not wizard.overdue_effects_account_id
            or not wizard.bank_account_id or not wizard.bank_expense_account_id
        ):
            raise UserError(_('Every account is mandatory.'))
        move_vals = {
            'ref':
                _('Past Due C/O %s - Line %s') %
