...
class RibaList(models.Model):
    ...
    state = fields.Selection([('draft', 'Draft'), ('accepted', 'Accepted'),
                              ('accredited', 'Credited'), ('paid', 'Paid'),
                              ('unsolved', 'Past Due'), ('cancel', 'Canceled')],
                             'State',
                             readonly=True,
                             default='draft')
    ...
    company_id = fields.Many2one(
        ...
        _company_default_get('riba.distinta')
    )
    ...
    def _onchange_date(self):
        if self.date_accepted and self.date_accreditation:
            if self.date_accepted > self.date_accreditation:
                raise UserError(
                    _("Credit date must be greater or equal to"
                      " acceptance date.")
                )
            ...
            for move_line in line.move_line_ids:
                ...
                if not line.invoice_number:
                    line.invoice_number = str(
                        move_line.move_line_id.invoice_id.move_id.name if move_line
            ...
            if (riba_line.acceptance_move_id and not riba_line.state == 'unsolved'):
                for line in riba_line.acceptance_move_id.line_ids:
                    payment_lines.extend([
                        _f for _f in
                        [rp.credit_move_id.id for rp in line.matched_credit_ids] if _f
                    ])
    ...
    def riba_line_settlement(self):
        for riba_line in self:
            ...
            settlement_move_line = move_line_model.search([
                ...
                ('move_id', '=', riba_line.acceptance_move_id.id), ('debit', '!=', 0)
            ])
